        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
          {phrase.map((symbol, index) => (
            <div key={`${symbol.id}-${index}`} className="flex flex-col items-center gap-1 animate-in fade-in zoom-in-95">
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white/20 rounded-lg flex items-center justify-center shadow-inner overflow-hidden"><SymbolDisplay symbol={symbol} /></div>
            </div>
          ))}
        </div>
      ) : (<div className="text-center text-slate-300/80 p-6"><Sparkles className="mx-auto h-8 w-8 sm:h-10 sm:w-10 mb-2" /><p className="font-medium text-sm sm:text-base">Comece a montar sua frase</p></div>)}
    </CardContent>
  </Card>
);

// --- ActionButtons com Copiar e Compartilhar ---
const ActionButtons = ({ onSpeak, onRemoveLast, onClear, onCopy, onShare }: { onSpeak: () => void; onRemoveLast: () => void; onClear: () => void; onCopy: () => void; onShare: () => void; }) => (
  <div className="grid grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-6">
    <Button onClick={onSpeak} className="h-20 sm:h-24 text-white shadow-xl col-span-2 flex-col gap-1 bg-gradient-to-br from-green-500 to-green-600 border-none"><PlayCircle className="h-7 w-7 sm:h-8 sm:w-8" /><span className="text-base sm:text-lg font-bold">Falar</span></Button>
    <Button onClick={onCopy} variant="outline" className="h-20 sm:h-24 text-white shadow-lg flex-col gap-1 bg-gradient-to-br from-sky-500 to-sky-600 border-none"><Copy className="h-6 w-6 sm:h-7 sm:w-7" /><span className="text-sm sm:text-md">Copiar</span></Button>
    <Button onClick={onShare} variant="outline" className="h-20 sm:h-24 text-white shadow-lg flex-col gap-1 bg-gradient-to-br from-purple-500 to-purple-600 border-none"><Share2 className="h-6 w-6 sm:h-7 sm:w-7" /><span className="text-sm sm:text-md">Partilhar</span></Button>
    {/* Os botões Apagar e Limpar foram movidos para uma linha secundária */}
    <Button onClick={onRemoveLast} variant="outline" className="h-16 sm:h-20 text-white shadow-lg flex-col col-span-2 bg-gradient-to-br from-amber-500 to-amber-600 border-none"><X className="h-5 w-5 sm:h-6 sm:h-6" /><span className="text-sm">Apagar Último</span></Button>
    <Button onClick={onClear} variant="destructive" className="h-16 sm:h-20 shadow-lg flex-col col-span-2 bg-gradient-to-br from-red-600 to-red-700 border-none"><Trash2 className="h-5 w-5 sm:h-6 sm:h-6" /><span className="text-sm">Limpar Tudo</span></Button>
  </div>
);
...
const SymbolGrid = ({ onSymbolClick }: { onSymbolClick: (symbol: DbSymbol) => void }) => {
  const data = useLiveQuery(async () => { const symbols = await db.symbols.toArray(); const categories = await db.categories.toArray(); const categoryColorMap = new Map(categories.map(cat => [cat.key, cat.color])); return { symbols, categoryColorMap }; }, []);
...
export const PhraseBuilder = ({ onBack }: PhraseBuilderProps) => {
  const [currentPhrase, setCurrentPhrase] = useState<DbSymbol[]>(() => { try { const savedPhrase = localStorage.getItem('currentPhrase'); return savedPhrase ? JSON.parse(savedPhrase) : []; } catch (error) { return []; } });
  const [voices, setVoices] = useState<SpeechSynthesisVoice[]>([]);
  const [selectedVoiceName, setSelectedVoiceName] = useState<string>('');
  const [inputMode, setInputMode] = useState<InputMode>('symbols');
  const { toast } = useToast();

  useEffect(() => { try { localStorage.setItem('currentPhrase', JSON.stringify(currentPhrase)); } catch (error) { console.error("Falha ao salvar a frase", error); } }, [currentPhrase]);
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 min-h-screen font-sans">
      ...
      <main>
        ...
        <ActionButtons onSpeak={speakPhrase} onRemoveLast={removeLastSymbol} onClear={clearPhrase} onCopy={copyPhrase} onShare={sharePhrase} />
