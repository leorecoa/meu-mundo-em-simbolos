...
import { PlayCircle, Trash2, X, Sparkles, Volume2, ChevronLeft, Keyboard as KeyboardIcon, Grid3x3, Send, Image } from 'lucide-react';
...
const SymbolDisplay = ({ symbol }: { symbol: DbSymbol }) => {
  ...
  useEffect(() => {
    if (symbol.image) {
      ...
      setImageUrl(url);
      // Limpa a URL quando o componente é desmontado para evitar vazamento de memória
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
          {phrase.map((symbol, index) => (
            <div key={`${symbol.id}-${index}`} className="flex flex-col items-center gap-1 animate-in fade-in zoom-in-95">
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white/20 rounded-lg flex items-center justify-center shadow-inner overflow-hidden">
...
const ActionButtons = ({ onSpeak, onRemoveLast, onClear }: { onSpeak: () => void; onRemoveLast: () => void; onClear: () => void; }) => (
  <div className="grid grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-6">
    <Button onClick={onSpeak} className="h-20 sm:h-24 text-white shadow-xl col-span-2 flex-col gap-1 bg-gradient-to-br from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 border-none"><PlayCircle className="h-7 w-7 sm:h-8 sm:w-8" /><span className="text-base sm:text-lg font-bold">Falar</span></Button>
    <Button onClick={onRemoveLast} variant="outline" className="h-20 sm:h-24 text-white shadow-lg flex-col gap-1 bg-gradient-to-br from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 border-none"><X className="h-6 w-6 sm:h-7 sm:w-7" /><span className="text-sm sm:text-md">Apagar</span></Button>
    <Button onClick={onClear} variant="destructive" className="h-20 sm:h-24 shadow-lg flex-col gap-1 bg-gradient-to-br from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 border-none"><Trash2 className="h-6 w-6 sm:h-7 sm:w-7" /><span className="text-sm sm:text-md">Limpar</span></Button>
  </div>
);
...
const VoiceSelector = ({ voices, selectedVoice, onVoiceChange }: { voices: SpeechSynthesisVoice[]; selectedVoice: string; onVoiceChange: (voiceName: string) => void; }) => (
  <div className="mb-4 sm:mb-6"><label htmlFor="voice-selector" className="flex items-center gap-2 text-slate-200 font-semibold mb-2 text-sm sm:text-base"><Volume2 className="h-5 w-5" />Voz do Aplicativo</label><select id="voice-selector" value={selectedVoice} onChange={(e) => onVoiceChange(e.target.value)} className="w-full p-2 sm:p-3 border-white/10 rounded-lg bg-black/30 shadow-md text-white text-sm sm:text-base focus:ring-2 focus:ring-blue-400 focus:outline-none" disabled={voices.length === 0}>{voices.length > 0 ? (voices.map(voice => <option key={voice.name} value={voice.name} className="text-black">{`${voice.name} (${voice.lang})`}</option>)) : (<option className="text-black">Carregando vozes...</option>)}</select><p className="text-xs text-slate-400 mt-2">A lista de vozes depende das instaladas no seu dispositivo.</p></div>
);
      ...
      <CardContent className="p-2 sm:p-4">
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 sm:gap-3">
          {filteredSymbols?.map(symbol => {
            ...
            return (
              <Button key={symbol.id} onClick={() => onSymbolClick(symbol)} variant="outline" className={`relative h-24 sm:h-28 text-lg sm:text-xl font-bold shadow-lg border-none transition-transform hover:scale-105 p-0 overflow-hidden ${colors.bg} ${colors.text} ${colors.hover}`}>
                <SymbolDisplay symbol={symbol} />
...
const NativeKeyboardInput = ({ onAddSymbol }: { onAddSymbol: (text: string) => void }) => {
  ...
  return (
    <Card className="shadow-xl bg-black/30 border-white/10"><CardContent className="p-4 flex items-center gap-2"><input type="text" value={text} onChange={(e) => setText(e.target.value)} onKeyDown={handleKeyDown} placeholder="Digite aqui e adicione à frase..." className="w-full h-12 text-lg font-medium bg-slate-100/80 rounded-lg px-4 shadow-inner text-slate-800 placeholder:text-slate-500 focus:ring-2 focus:ring-blue-400 focus:outline-none" /><Button onClick={handleAdd} className="h-12 bg-green-500 hover:bg-green-600 text-white shadow-lg" aria-label="Adicionar Palavra"><Send className="h-6 w-6" /></Button></CardContent></Card>
  );
};
...
export const PhraseBuilder = ({ onBack }: PhraseBuilderProps) => {
    ...
    if (typeof window !== 'undefined' && window.speechSynthesis) {
      const loadVoices = () => {
        ...
        if (preferredVoice && availableVoices.some(v => v.name === preferredVoice)) {
          ...
          if (googleVoice) { bestDefaultVoice = googleVoice; } else if (microsoftVoice) { bestDefaultVoice = microsoftVoice; }
  ...
  const addSymbol = useCallback((symbolOrText: DbSymbol | string) => { if (typeof symbolOrText === 'string') { const newSymbol: DbSymbol = { id: Date.now(), text: symbolOrText, categoryKey: 'custom' }; setCurrentPhrase(prev => [...prev, newSymbol]); } else { setCurrentPhrase(prev => [...prev, symbolOrText]); } }, []);
  const removeLastSymbol = useCallback(() => { setCurrentPhrase(prev => prev.slice(0, -1)); }, []);
  const clearPhrase = useCallback(() => { setCurrentPhrase([]); }, []);
  const speakPhrase = useCallback(() => { if (currentPhrase.length === 0) { toast({ title: "Frase vazia!", description: "Adicione símbolos para formar uma frase.", variant: "destructive" }); return; } const phraseText = currentPhrase.map(s => s.text).join(' '); window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(phraseText); const voiceToUse = voices.find(v => v.name === selectedVoiceName); if (voiceToUse) { utterance.voice = voiceToUse; } utterance.lang = 'pt-BR'; utterance.rate = 0.95; utterance.pitch = 1.0; window.speechSynthesis.speak(utterance); }, [currentPhrase, voices, selectedVoiceName, toast]);
