...
import { ChevronLeft, Trash2, PlusCircle, Edit, Check, ImageUp } from 'lucide-react';
...
// --- Componente Gerenciador de Categorias (sem alterações) ---
          ...
          {categories?.map(cat => (
            <li key={cat.id} className="flex items-center justify-between bg-black/20 p-2 rounded-md">
              {editingCategoryId === cat.id ? (
                <input type="text" value={editingCategoryName} onChange={(e) => setEditingCategoryName(e.target.value)} className="w-full bg-slate-100/80 rounded-lg px-2 text-slate-800" />
              ) : ( <span>{cat.name}</span> )}
              <div className="flex gap-2 ml-2">
                {editingCategoryId === cat.id ? (
                  <Button onClick={handleUpdateCategory} size="icon" className="bg-green-500"><Check size={16} /></Button>
                ) : (
                  <Button onClick={() => handleEditClick(cat)} size="icon" className="bg-blue-500"><Edit size={16} /></Button>
                )}
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button size="icon" variant="destructive" className="bg-red-600/80"><Trash2 size={16} /></Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent> <AlertDialogHeader> <AlertDialogTitle>Você tem certeza?</AlertDialogTitle> <AlertDialogDescription>Esta ação não pode ser desfeita. Isso vai apagar permanentemente a categoria e todos os símbolos dentro dela.</AlertDialogDescription> </AlertDialogHeader> <AlertDialogFooter> <AlertDialogCancel>Cancelar</AlertDialogCancel> <AlertDialogAction onClick={() => handleDeleteCategory(cat.id!)}>Apagar</AlertDialogAction> </AlertDialogFooter> </AlertDialogContent>
                </AlertDialog>
              </div>
            </li>
          ))}
...
// --- Componente Gerenciador de Símbolos com Upload de Imagem ---
const SymbolManager = () => {
  const { toast } = useToast();
  const data = useLiveQuery(async () => {
    const symbols = await db.symbols.orderBy('text').toArray();
  ...
  useEffect(() => {
    if (data?.categories.length && !selectedCategoryKey) {
      setSelectedCategoryKey(data.categories[0].key);
    }
  }, [data?.categories, selectedCategoryKey]);

  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      setNewSymbolImage(file);
  ...
  const handleAddSymbol = useCallback(async () => {
    ...
    if (!text || !selectedCategoryKey) {
      toast({ title: 'Erro', description: 'Texto e categoria são obrigatórios.', variant: 'destructive' });
      return;
    }
    
    const symbolToAdd: DbSymbol = { text, categoryKey: selectedCategoryKey };
    if (newSymbolImage) {
      symbolToAdd.image = newSymbolImage;
    }
  ...
  const handleDeleteSymbol = async (id: number) => {
    const symbol = await db.symbols.get(id);
    if (symbol) {
      await db.symbols.delete(id);
      toast({ title: 'Sucesso', description: `Símbolo "${symbol.text}" apagado.`, variant: 'destructive' });
    }
  };  

  const handleEditSymbolClick = (symbol: DbSymbol) => {
    setEditingSymbolId(symbol.id!);
    setEditingSymbolText(symbol.text);
  };
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 mb-4">
          <div className="flex flex-col gap-2">
            <input type="text" value={newSymbolText} onChange={(e) => setNewSymbolText(e.target.value)} placeholder="Texto do novo símbolo" className="w-full bg-slate-100/80 rounded-lg px-3 text-slate-800 h-10" />
            <select value={selectedCategoryKey} onChange={(e) => setSelectedCategoryKey(e.target.value)} className="w-full bg-slate-100/80 rounded-lg px-2 text-slate-800 h-10">
              {data?.categories.map(cat => <option key={cat.key} value={cat.key}>{cat.name}</option>)}
            </select>
          </div>
          <div className="flex flex-col items-center gap-2">
            <label className="cursor-pointer flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-slate-400 rounded-lg text-slate-400 hover:bg-slate-700/50 hover:border-slate-300 hover:text-slate-300">
              {previewUrl ? <img src={previewUrl} alt="Preview" className="w-full h-full object-cover rounded-lg"/> : <ImageUp size={32} />}
              <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
            </label>
            <Button onClick={handleAddSymbol} className="w-full bg-sky-500"><PlusCircle size={16} className="mr-2"/>Adicionar</Button>
          </div>
        </div>
          ...
          {data?.symbols.map(sym => (
            <li key={sym.id} className="flex items-center justify-between bg-black/20 p-2 rounded-md">
              <div className="flex items-center gap-2">
                {sym.image && <img src={URL.createObjectURL(sym.image)} className="w-8 h-8 rounded-sm object-cover" alt={sym.text} />}
                {editingSymbolId === sym.id ? (
                  <input type="text" value={editingSymbolText} onChange={(e) => setEditingSymbolText(e.target.value)} className="bg-slate-100/80 rounded-lg px-2 text-slate-800" />
                ) : ( <span>{sym.text}</span> )}
              </div>
              <div className="flex gap-2 ml-2">
                {editingSymbolId === sym.id ? (
                  <Button onClick={handleUpdateSymbol} size="icon" className="bg-green-500"><Check size={16} /></Button>
                ) : (
                  <Button onClick={() => handleEditSymbolClick(sym)} size="icon" className="bg-blue-500"><Edit size={16} /></Button>
                )}
                <AlertDialog>
                  <AlertDialogTrigger asChild><Button size="icon" variant="destructive" className="bg-red-600/80"><Trash2 size={16} /></Button></AlertDialogTrigger>
                  <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Você tem certeza?</AlertDialogTitle><AlertDialogDescription>Esta ação não pode ser desfeita. Isso vai apagar o símbolo permanentemente.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteSymbol(sym.id!)}>Apagar</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
                </AlertDialog>
              </div>
            </li>
          ))}
