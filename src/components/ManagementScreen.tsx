...
const SymbolManager = () => {
  ...
  return (
    <Card className="bg-white/80 backdrop-blur-sm border-gray-200/50 shadow-lg text-gray-800 mt-6">
              ...
              <ul className="space-y-2 max-h-96 overflow-y-auto pr-2" {...provided.droppableProps} ref={provided.innerRef}>
                {data?.symbols.map((sym, index) => (
                  <Draggable key={sym.id} draggableId={sym.id!.toString()} index={index} isDragDisabled={!isOrganizeMode}>
                    {(provided, snapshot) => (
                      <li ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`flex items-center justify-between bg-black/5 p-2 rounded-md ${snapshot.isDragging ? 'bg-blue-500 text-white' : ''}`}>
                        ...
                        {!isOrganizeMode && <div className="flex gap-2 ml-2">{editingSymbolId === sym.id ? (<Button onClick={handleUpdateSymbol} size="icon" className="bg-green-500 text-white"><Check size={16} /></Button>) : (<Button onClick={() => handleEditSymbolClick(sym)} size="icon" className="bg-blue-500 text-white"><Edit size={16} /></Button>)}{sym.image && <><input type="file" accept="image/*" ref={fileInputRef} onChange={(e) => e.target.files && handleUpdateImage(sym.id!, e.target.files[0])} className="hidden" /><Button onClick={() => fileInputRef.current?.click()} size="icon" className="bg-purple-500 text-white"><RefreshCcw size={16} /></Button><Button onClick={() => handleRemoveImage(sym.id!)} size="icon" className="bg-orange-500 text-white"><ImageOff size={16} /></Button></>}<AlertDialog><AlertDialogTrigger asChild><Button size="icon" variant="destructive" className="bg-red-600/80 text-white"><Trash2 size={16} /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Você tem certeza?</AlertDialogTitle><AlertDialogDescription>Esta ação não pode ser desfeita. Isso vai apagar o símbolo permanentemente.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteSymbol(sym.id!)}>Apagar</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>}
                      </li>
                    )}
                  </Draggable>
                ))}
...
export const ManagementScreen = ({ onBack }: ManagementScreenProps) => {
  return (
    <div className="p-2 sm:p-4 md:p-6 min-h-screen font-sans"><header className="flex items-center justify-between mb-4"><Button variant="ghost" onClick={onBack} className="flex items-center gap-1 text-sm sm:text-base text-gray-800 font-semibold"><ChevronLeft className="h-5 w-5" />Voltar</Button><h1 className="text-lg sm:text-xl font-bold text-gray-700">Meu Painel</h1><div className="w-16 sm:w-24"></div></header>
      <main>
        <CategoryManager />
        <SymbolManager />
        <BackupManager />
