...
// --- Componentes atualizados para usar o Contexto de Perfil ---
...
const SymbolManager = () => {
  ...
  const data = useLiveQuery(async () => {
    ...
    const symbols = await db.symbols.where({ profileId: activeProfileId }).filter(s => s.text.toLowerCase().includes(lowerSearchTerm)).toArray();
  ...
  const handleAddSymbol = useCallback(async () => {
    ...
    const symbolToAdd: DbSymbol = { profileId: activeProfileId, text, categoryKey: selectedCategoryKey };
  ...
  const handleUpdateSymbol = async () => {
    if (!editingSymbolId || !activeProfileId) return;
    const text = editingSymbolText.trim();
    if (!text) { toast({ title: 'Erro', description: 'O texto não pode ser vazio.', variant: 'destructive' }); return; }
    if ((await db.symbols.get(editingSymbolId))?.profileId === activeProfileId) { await db.symbols.update(editingSymbolId, { text }); toast({ title: 'Sucesso', description: 'Símbolo atualizado.' }); }
