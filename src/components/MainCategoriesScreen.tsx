...
export const MainCategoriesScreen = ({ onNavigateToCategory, onNavigateToPhrase, onNavigateToMyAT }: MainCategoriesScreenProps) => {
  // Hook para buscar as categorias em tempo real do banco de dados local
  const categories = useLiveQuery(() => db.categories.toArray(), []);

  // A tela de carregamento agora é exibida enquanto os dados do Dexie são carregados
  if (!categories) {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {categories.map(category => {
          // Filtra a categoria 'geral' para não ser exibida aqui
