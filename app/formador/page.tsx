...
// --- Interface para os Símbolos ---
interface Symbol {
  id: string;
  text: string;
  image?: string; // Futuro uso para imagens personalizadas
}
...
export default function FormadorFrasesPage() {
  ...
  // Carregamento "paciente" e seguro das vozes do navegador
  useEffect(() => {
  ...
  const speakPhrase = () => {
    if (currentPhrase.length === 0) {
      toast({
        title: "Frase vazia!",
        description: "Adicione símbolos para começar.",
        variant: "destructive",
      });
      return;
    }
  ...
  // --- Dados de Exemplo para Funcionar Imediatamente ---
  ...
  return (
    <main className="p-4 md:p-6 bg-slate-50 min-h-screen font-sans">
      {/* 1. Área de Visualização da Frase */}
      ...
      {/* 2. Painel de Controle com Botões Grandes */}
      ...
      {/* 4. Grid de Símbolos Selecionáveis */}
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
            {exampleSymbols.map(symbol => (
              <Button
                ...
                variant="outline"
                className="h-28 text-xl font-bold text-slate-800 shadow-sm hover:bg-blue-50 hover:border-blue-400"
              >
                {symbol.text}
              </Button>
            ))}
